...
class Fractal:
    ...
    def draw_fractal(self, upper_left_x, upper_left_y, square_size, levels_of_recursion):
        """ Draw fractal with levels_of_recursion in square whose upper level corner is
            (upper_left_x, upper_left_y), whose size is square_size. 
        """
        if levels_of_recursion == 0:
            return
        else:
            self.canvas.create_rectangle(upper_left_x, upper_left_y, 
                                        upper_left_x + square_size // 2,
                                        upper_left_y + square_size // 2,
                                        fill = "black")
            self.draw_fractal(upper_left_x + square_size // 2,
                            upper_left_y,
                            square_size // 2, levels_of_recursion - 1)
